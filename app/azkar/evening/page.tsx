...
export default function eveningAzkarPage() {
  ...
  const [count, setCount] = useState(eveningAzkar[0].count);

  const current = eveningAzkar[currentZekr];
  ...
  function handleDone() {
    ...
    if (currentZekr < eveningAzkar.length - 1) {
      const next = currentZekr + 1;
      setCurrentZekr(next);
      setCount(eveningAzkar[next].count);
      return;
    }
  ...
  const finished =
    currentZekr === eveningAzkar.length - 1 &&
    count === 0;
      ...
      <main className="min-h-screen bg-[#F7F6F2] flex items-center justify-center px-5">
        <div className="max-w-xl rounded-[35px] bg-white p-10 text-center shadow-xl">
          ...
          <p className="mt-4 text-lg leading-9 text-slate-600">
            لقد أتممت أذكار المساء كاملة.
          </p>

          <button
            onClick={() => {
              setCurrentZekr(0);
              setCount(eveningAzkar[0].count);
            }}
