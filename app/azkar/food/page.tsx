...
import foodAzkar from "@/data/foodAzkar";

export default function FoodAzkarPage() {
  const [currentZekr, setCurrentZekr] = useState(0);
  const [count, setCount] = useState(foodAzkar[0].count);

  const current = foodAzkar[currentZekr];
  ...
  function handleDone() {
    ...
    if (currentZekr < foodAzkar.length - 1) {
      const next = currentZekr + 1;
      setCurrentZekr(next);
      setCount(foodAzkar[next].count);
      return;
    }
  ...
  const finished =
    currentZekr === foodAzkar.length - 1 &&
    count === 0;
      ...
      <main className="min-h-screen bg-[#F7F6F2] flex items-center justify-center px-5">
        <div className="max-w-xl rounded-[35px] bg-white p-10 text-center shadow-xl">
          ...
          <p className="mt-4 text-lg leading-9 text-slate-600">
            لقد أتممت أذكار الطعام كاملة.
          </p>

          <button
            onClick={() => {
              setCurrentZekr(0);
              setCount(foodAzkar[0].count);
            }}
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 h-3 overflow-hidden rounded-full bg-slate-200">
          ...
          <div
            ...
            style={{
              width: `${((currentZekr + 1) / foodAzkar.length) * 100}%`,
            }}
          />
...
>
          ...
          <div className="mb-8 flex items-center justify-between">
            ...
            <div className="rounded-full bg-emerald-100 px-5 py-2 font-bold text-emerald-800">
              الذكر {currentZekr + 1} من {foodAzkar.length}
            </div>
